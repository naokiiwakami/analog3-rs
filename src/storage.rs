...
impl<'a> Storage<'a> {
    ...
    fn load(&mut self, address: u16, value_type: ValueType) -> Result<Value, Error> {
        match value_type {
            ...
            ValueType::I8 => self.load_u8(address).map(|v| Value::I8(v as i8)),
            ValueType::I16 => self.load_u16(address).map(|v| Value::I16(v as i16)),
            ValueType::I32 => self.load_u32(address).map(|v| Value::I32(v as i32)),
    ...
    pub async fn save(&mut self, address: u16, value: Value) -> Result<(), Error> {
        match value {
            ...
            Value::I8(data) => self.save_u8(address, data as u8).await,
            Value::I16(data) => self.save_u16(address, data as u16).await,
            Value::I32(data) => self.save_u32(address, data as u32).await,
